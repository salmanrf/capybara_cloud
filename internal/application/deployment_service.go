...
var root_localfs_artifacts = "/tmp/capybara_cloud"
...
func NewDeploymentService(ctx context.Context, app_service Service, deployment_repository DeploymentRepository) DeploymentService {
...
func (s *deployment_service) Deploy(user_id string, app_id string, bundle_file_headers *multipart.FileHeader, bundle_file multipart.File) (*database.ApplicationDeployment, error) {
	...
	artifact_path, err := saveDeployArtifacts(*app, bundle_file_headers, bundle_file)
...
func getArtifactDirPath(storage_service string, app database.FindOneApplicationWithProjectMemberRow) string {
	_ = storage_service
	...
	full_path := fmt.Sprintf("%s/artifact-%s-%s", root_localfs_artifacts, appnameslug, deploy_datestr) 
