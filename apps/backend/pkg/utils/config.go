...
func (c *Config) Validate() error {
	...
	if c.AUTH_JWT_ISSUER == "" {
		errs = append(errs, errors.New("AUTH_JWT_ISSUER is required"))
	}
	if c.AUTH_JWT_AUDIENCE == "" {
		errs = append(errs, errors.New("AUTH_JWT_AUDIENCE is required"))
	}
...
func SetConfig(newconf Config) {
	app_cfg = Config{
		...
		AUTH_JWT_ISSUER: 					newconf.AUTH_JWT_ISSUER,
		AUTH_JWT_AUDIENCE: 				newconf.AUTH_JWT_AUDIENCE,
